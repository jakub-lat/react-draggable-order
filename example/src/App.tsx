...
import { OrderGroup, useOrder } from 'react-orderable';
import './style.css';
...
function Container({data, index, onMove}: IProps) {
    ...
    return (
        <div ref={wrapperRef} style={{margin: '20px 0'}}>
            <div
                ref={elementRef}
                style={{
                    ...elementStyle,
                ...
                {list
                    .map((x, i) => (
                        <Container
                            ...
                            data={x}
                            onMove={(to) => {
                                console.log(`move ${i} to ${to}`);
                                setList(arrayMove(list, i, to));
                            }}
                        />
                    ))
                    .reduce<React.ReactNodeArray>((acc, x, i) => (!acc ? [sep(i), x] : [...acc, sep(i), x]), null)}
            </OrderGroup>
        </div>
    );
}
